/**
 * Dump FR Tier B rows (stripped diacritics) with full context for hand-rewrite.
 *
 * Run:
 *   source .env && export PUBLIC_SUPABASE_URL SUPABASE_SERVICE_ROLE_KEY && npx tsx scripts/dump-tier-b-fr.ts
 */
...
const FR_DIACRITICS = 'àâäçéèêëîïôöùûüÿœæÀÂÄÇÉÈÊËÎÏÔÖÙÛÜŸŒÆ';
const MIN_LEN = 40;

function hasDiacritic(text: string): boolean {
  for (const ch of text) if (FR_DIACRITICS.includes(ch)) return true;
  return false;
}
...
async function fetchFR(): Promise<Row[]> {
  const all: Row[] = [];
  let from = 0;
  while (true) {
...
(async () => {
  ...
  // Join recipes for slug + EN source
  ...
  console.log(`Joined ${rmap.size} recipes`);
  for (const f of flagged) {
    const r = rmap.get(f.recipe_id);
    if (r) {
      f.slug = r.slug;
      f.en_title = r.title;
      f.en_description = r.description;
    }
  }
  ...
  writeFileSync('data/tier-b-fr-dump.json', JSON.stringify(flagged, null, 2));
  console.log(`Wrote ${flagged.length} FR rows to data/tier-b-fr-dump.json`);
})();
