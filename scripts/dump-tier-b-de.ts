/**
 * Dump DE Tier B rows (stripped diacritics) for hand-rewrite.
 *
 * Run:
 *   source .env && export PUBLIC_SUPABASE_URL SUPABASE_SERVICE_ROLE_KEY && npx tsx scripts/dump-tier-b-de.ts
 */
...
const DE_DIACRITICS = 'äöüßÄÖÜ';
const MIN_LEN = 40;

function hasDiacritic(text: string): boolean {
  for (const ch of text) if (DE_DIACRITICS.includes(ch)) return true;
  return false;
}
...
(async () => {
  ...
  writeFileSync('data/tier-b-de-dump.json', JSON.stringify(flagged, null, 2));
  console.log(`Wrote ${flagged.length} DE rows to data/tier-b-de-dump.json`);
})();
