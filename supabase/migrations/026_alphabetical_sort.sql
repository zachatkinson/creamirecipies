-- Add alphabetical (A-Z) sort option to recipe listing
...
BEGIN
  ...
  RETURN QUERY
  WITH filtered AS (
    SELECT r.id, r.published_at, r.avg_rating, r.rating_count, r.prep_time_minutes, r.title
    FROM recipes r
  ...
  ORDER BY
    ...
    CASE WHEN p_sort = 'alphabetical' THEN f.title END ASC NULLS LAST,
    f.id ASC
  LIMIT p_limit
  OFFSET p_offset;
END;
